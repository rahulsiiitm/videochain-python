...
\section{Detailed Architectural Blueprint}

While the five-layer model defines the logical boundaries, the Detailed Architecture Blueprint (Figure~\ref{fig:detailed_arch}) illustrates the physical data flow and component orchestration. The Media Gateway acts as the primary buffer, decoupling the asynchronous node scheduler from the synchronous video decoder. Every expert node (VLM, ASR, OCR, Tracker) executes in isolation, communicating only through the IRIS context dictionary. This ensures that a failure in one modality (e.g., a noisy audio track failing to transcribe) does not halt the visual or textual extraction pipelines.

\newgeometry{left=0.2cm, right=0.2cm, top=0.2cm, bottom=0.2cm}
\begin{sidewaysfigure}
\centering
\includegraphics[width=0.90\textheight]{detailed_architecture.pdf}
\caption{Detailed Architecture Blueprint --- Component Interaction and Data-Flow Paths}
